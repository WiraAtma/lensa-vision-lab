from pathlib import Path

import torch
from fastapi import HTTPException, UploadFile, status
from PIL import Image
from torchvision import transforms

from app.architectures.efficientnet_b0 import efficientnet_b0
from app.core.formatter import format_class_name
from app.core.model import create_model_loader
from app.core.predict import predict_image

BASE_DIR = Path(__file__).resolve().parents[2]

MODEL_PATH = (BASE_DIR / "models" / "facial_expression_classification" / "efficientnet_b0_facial_expressions_classification_model_v1.pth")

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

get_model = create_model_loader(
  model_builder=lambda: efficientnet_b0(num_classes=7),
  model_path=MODEL_PATH,
  device=DEVICE,
  name="FACIAL EXPRESSIONS MODEL"
)

predict_transform = transforms.Compose([
  transforms.Resize((224, 224)),
  transforms.ToTensor(),
  transforms.Normalize(
    mean=[0.5078, 0.5078, 0.5078],
    std=[0.2528, 0.2528, 0.2528]
  )
])

class FacialExpressionsClassificationService:
  async def predict(self, file: UploadFile):

    if not file.content_type.startswith("image/"):
      raise HTTPException(
        status_code=status.HTTP_400_BAD_REQUEST,
        detail="File must be an image.",
      )

    try:
      image = Image.open(file.file).convert("RGB")

    except Exception:  # noqa: BLE001
      raise HTTPException(
        status_code=status.HTTP_400_BAD_REQUEST,
        detail="Invalid image file.",
      )

    model, class_names = get_model()

    pred, confidence, top_indices, top_probs = predict_image(
      model=model,
      image=image,
      device=DEVICE,
      transform=predict_transform,
    )

    return {
      "prediction": format_class_name(class_names[pred]),
      "confidence": round(confidence * 100, 2),
      "probabilities": [
        {
          "class": class_names[idx],
          "probability": round(prob * 100, 2),
        }
        for idx, prob in zip(top_indices, top_probs)
      ],
    }