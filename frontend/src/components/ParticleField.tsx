...
const ParticleField = ({ position = 'top', className = '' }: ParticleFieldProps) => {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            for (const p of dust) {
                ...
                ctx.fillStyle = `rgba(255, 101, 63, ${p.opacity * 0.75})`;
                ctx.fill();
            }
            ...
            for (const c of CONSTELLATIONS) {
                ...
                for (const s of c.stars) {
                    ...
                    ctx.fillStyle = `rgba(255, 101, 63, ${twinkle + 0.12})`;
                    ctx.fill();
                }
            }
            ...
            for (const s of stars) {
                ...
                glow.addColorStop(0, `rgba(255, 101, 63, ${opacity * 0.18})`);
                glow.addColorStop(0.5, `rgba(255, 101, 63, ${opacity * 0.06})`);
                glow.addColorStop(1, 'rgba(255, 101, 63, 0)');
                ctx.beginPath();
                ...
                ctx.fillStyle = `rgba(217, 75, 42, ${opacity * 0.45})`;
                ctx.fill();

                // Cross-shaped twinkle rays
                const rayLen = s.r * 3 * ((pulse + 1) / 2);
                ctx.strokeStyle = `rgba(217, 75, 42, ${opacity * 0.18})`;
                ctx.lineWidth = 0.5;
                ctx.beginPath();
    ...
    return (
        <canvas
            ref={canvasRef}
            className={`pointer-events-none fixed inset-x-0 ${positionClasses} z-0 h-[25vh] w-full ${className}`}
            aria-hidden="true"
        />
    );
};
